...
export async function getCreatorProfile(username) {
  ...
  if (!hasSupabaseConfig) {
    return { profile: { username: handle, display_name: handle }, videos: fallbackContent.filter((item) => item.username === handle) }
  }
  return apiRequest(`/profiles/${encodeURIComponent(handle)}`)
}
