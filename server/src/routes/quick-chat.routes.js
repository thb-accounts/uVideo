...
router.post('/suggestions', requireAuth, async (req, res) => {
  ...
  const existing = await prisma.quickChatSuggestion.findUnique({ where: { normalized: profanity.normalized } })
  if (existing) return res.status(409).json({ message: 'Duplicate suggestion' })
