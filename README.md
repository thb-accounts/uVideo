...
- Link-based video publishing with a companion direct-upload service.
...
1. Install dependencies:
   ...
   npm install
   ```
2. Copy `.env.example` to `.env` and add the desired Supabase configuration.
...
Authenticated creators publish a direct `.mp4` URL. Creators who need to upload a local file can use [server.unrealcake8.site](https://server.unrealcake8.site) from the upload page, then paste the generated link into UVideo.
...
2. Configure the Supabase and auth redirect environment variables from `.env.example`.
